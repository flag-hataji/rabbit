...
*/
	...
	*/
	...
	global $cPostgres,$cDebug,$libConvert;
		...
		$query  = "SELECT * ";
 		$query .= " FROM td_error_log_itm WHERE user_id = {$_SESSION['user']['user_id']} ";
 		$query .= " ORDER BY error_count DESC , date_insert LIMIT 500 ";

		//count用
		$query2  = "SELECT count(*) ";
 		$query2 .= " FROM td_error_log_itm WHERE user_id = {$_SESSION['user']['user_id']} ";
	...
	*/
	...
	global $cPostgres;
		...
		$query  = "DELETE FROM td_error_log_itm WHERE user_id = {$_SESSION['user']['user_id']} ";
