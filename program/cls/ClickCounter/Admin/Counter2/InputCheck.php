...
class InputCheck extends Web
{
  ...
  function setCheck()
  {
    ...
    // category
    switch( $this->bean->getInput('category_type') ){
      case 1 :
        if(! $this->check->isInput( $this->bean->getInput('category_id') ) ){
          $this->setError("category_id", $this->bean->getInput('category_id'), "グループを選択してください。");
        }
        break ;
      case 2 :
        if(! $this->check->isInput( $this->bean->getInput('category_name') ) ){
          $this->setError("category_name", $this->bean->getInput('category_name'), "新規グループ名を記述してください。");
        }
        break ;
      default :
        $this->setError("category_type", $this->bean->getInput('category_type'), "グループを選択してください。");
    }
