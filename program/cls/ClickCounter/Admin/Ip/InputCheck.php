...
class InputCheck extends Web
{
  ...
  function setCheck()
  {
    ...
    // ip
    if(! $this->check->isInput( $this->bean->getInput('ip') ) ){
      $this->setError("ip", $this->bean->getInput('ip'), "IPを入力してください。");
    }

    // title
    if(! $this->check->isInput( $this->bean->getInput('title') ) ){
      $this->setError("title", $this->bean->getInput('title'), "ＩＰ名称をご入力ください。");
    }
