...
<?PHP
  ...
  class Sender{
    ...
    function Sender(){
//mail("masaki@itm.ne.jp","Sender","RUN");
...
*/
    ...
    function sendMailFinish($subject=False,$message=False){
      ...
      $subjectUser   = "[itm-asp]メール配信のご予約を承りました";
      $messageUser  = file_get_contents($this->defineS['PgRootMail']."userReserve.txt");

      $subjectMaster  = "[itm-asp]メール配信のご予約がありました\n";
    ...
    function sendMailError($num=False,$ex=False){
      ...
      if($subjectMaster!="" && $messageMaster!=""){
        //$this->sendMailMaster($subjectMaster,$messageMaster);
        $Mail->normalMb_send_mail('info@itm-asp.com', 'masaki@itm.ne.jp,hataji@itm.ne.jp,msystem@itm.ne.jp', $subject, $message );
      }


      $subjectuser="【緊急】[itm-asp]メール送信ご予約の受付が出来ませんでした";
    ...
    function sendMailMaster($subject=False,$message=False){
      ...
      $Mail->normalMb_send_mail('info@itm-asp.com', 'masaki@itm.ne.jp,hataji@itm.ne.jp', $subject, $message );
