...
class Viewer extends Html {
  ...
  function showHidden(){
    ...
    switch($this->pageS['place']){
      ...
      case 'rewrite' : 
      case 'write' : 
        if( $this->Manager->writeS['inquiry_id']!="" ){
          echo "<input type='hidden' name='inputS[inquiry_id]' value='{$this->Manager->writeS['inquiry_id']}'>\n";
          if(_DEBUG_){
            echo "&lt; input type='hidden' name='inputS[inquiry_id]' value='{$this->Manager->writeS['inquiry_id']}'&gt;<br>\n";
          }
        }
        break;
    }
