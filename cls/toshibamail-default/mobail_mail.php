...
/*
  ...
  $subject= "空メールエラー";
  $content= "ヘッダー=".$text."空メールでrabbit-mail-xxxxxx@rabbit-mail.jpに該当しなかった";
...
//foreach($headers as $key=>$value){
  ...
  $pat = "/rabbit-mail\-\d+@rabbit-mail.jp/";
...
if($num==0){
  ...
  mb_language("Ja") ;
  //mb_internal_encoding("EUC-JP") ;
  $mailto="niki@itm.ne.jp,hataji@itm.ne.jp";
  $subject= "空メールエラー";
  $content= "====================ヘッダ情報===================\n".
            $text.
            "====================ヘッダ情報ここまで============\n".
            "空メールでrabbit-mail-xxxxxx@rabbit-mail.jpに該当しなかった";
  //$content= "ヘッダー=".$text."空メールでrabbit-mail-xxxxxx@rabbit-mail.jpに該当しなかった";
  $mailfrom="From:".$headers['From'];
  mb_send_mail($mailto,$subject,$content,$mailfrom);
  exit;
      ...
      if($out_cnt!=in_cnt){
        if($to_arr[$out_cnt]!=$to_arr[$in_cnt]){
          ...
          $content= "====================ヘッダ情報===================\n".
                    $text.
                    "====================ヘッダ情報ここまで============\n".
                    "空メールのヘッダーでrabbit-mail-xxxxxx@rabbit-mail.jpのメールアドレス２つ以上あった";
          //$content= "ヘッダー=".$text."空メールのヘッダーでrabbit-mail-xxxxxx@rabbit-mail.jpのメールアドレス２つ以上あった";
          $mailfrom="From:".$headers['From'];
          mb_send_mail($mailto,$subject,$content,$mailfrom);
          exit;
        }
...
if($sql_data['html_flag']==1&&$pc_flag==1){
				$html_mess    = $sql_data['html_mess'];
    		//テキストメッセージのユーザータグ、改行を置換
    		$html_mess        = str_replace("%name%","",$html_mess);
    		$html_mess        = str_replace("%name_family%","",$html_mess);
    		$html_mess        = str_replace("%name_first%","",$html_mess);
    		$html_mess        = str_replace("%name_family%","",$html_mess);
    		$html_mess        = str_replace("%email%",$recipients,$html_mess);
    		$html_mess        = ereg_replace("\n","",$html_mess);
				//HTMLメール送信
				$myMail->htmlMail($transmit_mailadd,$recipients,$subject,"",$html_mess);
        //$myMail->normalMb_send_mail($transmit_mailadd,$recipients,$subject,"",$return_err);
        exit;
}
    ...
    $content= "====================ヘッダ情報===================\n".
              $text.
              "====================ヘッダ情報ここまで============\n".
              "IDエラー。\nIDが存在しないか、フォームは作成したがサンキューメール設定がされてない可能性がある。";
    $mailfrom="From:".$headers['From'];
    mb_send_mail($mailto,$subject,$content,$mailfrom);
    exit;
  }
