...
*/
	//設定ファイル読み込み
	require_once ('../../cls/test/user/setup.php');
...
//			$aData[$key] = str_replace('"','\"',$value);
	...
	*/
	...
	global $cDb;
		...
		$query  = "SELECT * FROM td_mmail_user WHERE mmail_user_id = {$_GET['id']}";
		$dataS = $cDb->executeQuery($query);
		while ($getData = pg_fetch_array($dataS)){
			list($getData['tel1'],$getData['tel2'],$getData['tel3']) = explode("-",$getData['tel']);
			$aData = $getData; 
		...
		if($aData){
			...
			require_once('../rewrite/login.html');
		  //header("location:http://".$_SERVER["HTTP_HOST"]."/guide/member/rewrite/login.html" );
			exit;
		}
	}
