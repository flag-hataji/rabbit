...
//}
	...
	//設定ファイル読み込み
	require_once ('../../cls/test/member_list/setup.php');
...
*/
	...
	*/
	...
	if(isset($_POST['del'])){
		$mmail_member_id = key($_POST['del']) ;
		delData($mmail_member_id);
	}
	...
	*/
	...
	global $cPostgres,$cDebug,$libConvert;
		...
		$query  = "SELECT mmail_member_id , td_mmail_user.name AS user_name , td_mmail_member.name AS member_name , email ";
 		$query .= " FROM td_mmail_member JOIN td_mmail_user ON td_mmail_member.user_id = td_mmail_user.mmail_user_id ORDER BY user_id  ";
	...
	*データ削除
	*/
	function delData($mmail_member_id){
	global $cPostgres;

		$query = "DELETE FROM td_mmail_member WHERE mmail_member_id = {$mmail_member_id}";
		if(_DEBUG_){ echo $query; }
